...
async function run() {
	...
	const workspaceDirs = workspaces.filter((ws) => !excluded.has(ws)).filter((ws) => !!ws);
	...
	// Run prettier from root (formatting now runs from root, not per-project)
	if (!args.skipPrettier && !args.dryRun) {
		await runShellCmd('npm', ['run', 'prettier'], rootDir, args);
	}
...
function printHelp() {
	console.log(
		`Usage: node scripts/upgrade-versions.mjs [options]\n\n` +
